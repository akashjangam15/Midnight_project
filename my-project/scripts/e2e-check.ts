/**
 * End-to-end smoke check for my-project.
 *
 * Reconnects to the deployed counter, reads its PUBLIC ledger off the chain,
 * and exits 0 on success. Read-only: it never balances or submits a
 * transaction, so it costs no DUST.
 *
...
import { createCounterPrivateState, witnesses } from '../src/witnesses';
...
async function main() {
  ...
  try {
    await findDeployedContract(providers, {
      ...
      initialPrivateState: createCounterPrivateState(1, 'counter e2e-check note'),
    });
    connected = true;
  } catch (err: any) {
  ...
  // 4. Read the on-chain PUBLIC ledger via the public data provider — proves
  ...
  const publicLedger = Counter.ledger(onChainState.data);
  if (typeof publicLedger.count !== 'bigint') {
    await walletCtx.wallet.stop();
    fail(`ledger decoded unexpectedly: ${JSON.stringify(publicLedger.count)}`);
  }
