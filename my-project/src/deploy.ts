 ...
 *
 * Differences from a witness-free deploy (e.g. the hello-world template):
 *   - the compiled contract is built with `CompiledContract.withWitnesses`
 *     instead of `withVacantWitnesses`,
 *   - an initial private state accompanies it, because counter.compact's
 *     witnesses read `privateStep` / `privateMessage` from it.
 */
...
// Identifier under which this contract's private state is stored. The CLI must
// use the same id so it reconnects to the same private state.
...
//
// The step and note the contract will start with. Both are PRIVATE: they are
// written to the local encrypted private-state store, never to the chain. The
// deployed contract's public ledger starts at count = 0, updateCount = 0,
// publishedMessage = "" regardless of what is set here.
//
// Override at deploy time with COUNTER_STEP / COUNTER_MESSAGE.
...
// Typed as the generated module so `withWitnesses` can check the witness
// shapes against the compiled contract, while the import stays dynamic (and
// therefore tolerant of the artifacts being absent until compile runs).
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy my-project counter to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // initialPrivateState. counter.compact declares a no-argument
      // constructor, so args stays empty; its witnesses read the private
      // state below (still private — nothing here is published by deploying).
  ...
  // Read the on-chain state back to confirm what the world can see. The
  // indexer may need a moment to pick the deploy up, so this is best-effort:
  // a miss here does not mean the deploy failed.
